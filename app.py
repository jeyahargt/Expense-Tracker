...
if connection:
    ...
    query_options = [
        ...
        "Classification of Payment Mode",
    ...
    if selected_query == "Total Amount Spent in Each Category":
    ...
    elif selected_query == "Month Having Highest Spending in Transportation & Entertainment":
     st.write("### Month Having Highest Spending in Transportation & Entertainment")
     query = """
        WITH RankedExpenses AS (
            SELECT 
                ...
                RANK() OVER (PARTITION BY Category ORDER BY SUM(Amount_paid) DESC) AS `rank`
            FROM all_expenses
            WHERE Category IN ('Transportation', 'Entertainment')
     ...
     df = execute_query(query, connection)
     if df is not None:
        df['Month'] = df['Month'].apply(lambda x: pd.to_datetime(f'2024-{x:02d}-01').strftime('%B'))  # Convert to full month name
        st.dataframe(df)
     ...
     if df is not None:
    ...
    elif selected_query == "Classification of Payment Mode":
        st.write("### Classification of Payment Mode")
        query = """
        ...
        if df is not None:
            fig, ax = plt.subplots()
            ax.pie(df["Frequency"], labels=df["Payment_mode"], autopct="%.2f %%", startangle=90, colors=plt.cm.Paired.colors)
            ax.axis("equal")
            st.pyplot(fig)
